...
// - 有時間戳記+有備註的舊格式資料（新增source欄位之前寫入的，會被誤標成manual預設值，用這個特徵抓出來）
async function deleteLegacyBackfill() {
  const b = await pool.query(`DELETE FROM buys WHERE source IN ('backfill','correction') OR (signal_time IS NOT NULL AND note IS NOT NULL)`);
  const s = await pool.query(`DELETE FROM sells WHERE source IN ('backfill','correction') OR (signal_time IS NOT NULL AND note IS NOT NULL)`);
  return { buys: b.rowCount, sells: s.rowCount };
}
